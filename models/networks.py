...
def define_D(input_nc, ndf, which_model_netD,
    ...
    if which_model_netD == 'basic':
        ...
        netD = FusionDiscriminator(input_nc, ndf)
    else:
...
class FusionDiscriminator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, num_downs=4, spec_norm=True):
        ...
        self.bridge = Conv_residual_conv(self.out_dim * 2, self.out_dim * 4, act_fn, spec_norm)
        # self.bridge = Conv_residual_conv(self.out_dim * 4, self.out_dim * 8, act_fn, spec_norm)


        # output

        if spec_norm:
            self.out = spectral_norm(nn.Conv2d(self.out_dim*4, 1, kernel_size=3, stride=1, padding=1))
        else:
            self.out = nn.Conv2d(self.out_dim*4, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, input):
        ...
        # down_3 = self.down_3(pool_2)  # 50x50x256
        # pool_3 = self.pool_3(down_3)  # 50x50x256

        bridge = self.bridge(pool_2)  # 25x25x512

        out = self.out(bridge)
        # out = nn.Sigmoid(out)  # 400x400x3 output
...
class FusionGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, num_downs=4, spec_norm=True):
        ...
        # self.down_1 = DenseBlock(self.in_dim, self.out_dim, act_fn, spec_norm)
        # self.pool_1 = maxpool()
        # self.down_2 = DenseBlock(self.out_dim, self.out_dim * 2, act_fn, spec_norm)
        # self.pool_2 = maxpool()
        # self.down_3 = DenseBlock(self.out_dim * 2, self.out_dim * 4, act_fn, spec_norm)
        # self.pool_3 = maxpool()
        ...
        # self.attn_1 = Self_Attn(self.out_dim, 'relu')
        # self.attn_2 = Self_Attn(self.out_dim * 2, 'relu')
        # self.attn_3 = Self_Attn(self.out_dim * 4, 'relu')

        self.down_4 = Conv_residual_conv(self.out_dim * 4, self.out_dim * 8, act_fn, spec_norm)
        self.pool_4 = maxpool()
        # self.down_5 = Conv_residual_conv(self.out_dim * 8, self.out_dim * 8, act_fn, spec_norm)
        # self.pool_5 = maxpool()

        # bridge

        # self.bridge = DenseBlock(self.out_dim * 4, self.out_dim * 8, act_fn, spec_norm)
        # self.bridge = Conv_residual_conv(self.out_dim * 4, self.out_dim * 8, act_fn, spec_norm)

        self.bridge = Conv_residual_conv(self.out_dim * 8, self.out_dim * 8, act_fn, spec_norm)

        # decoder

        # self.deconv_1 = conv_trans_block(self.out_dim * 8, self.out_dim * 4, act_fn_2, spec_norm)
        # self.up_1 = Conv_residual_conv(self.out_dim * 4, self.out_dim * 4, act_fn_2, spec_norm)
        # self.deconv_2 = conv_trans_block(self.out_dim * 4, self.out_dim * 2, act_fn_2, spec_norm)
        # self.up_2 = Conv_residual_conv(self.out_dim * 2, self.out_dim * 2, act_fn_2, spec_norm)
        # self.deconv_3 = conv_trans_block(self.out_dim * 2, self.out_dim, act_fn_2, spec_norm)
        # self.up_3 = Conv_residual_conv(self.out_dim, self.out_dim, act_fn_2, spec_norm)

        # self.deconv_1 = conv_trans_block(self.out_dim * 8, self.out_dim * 4, act_fn_2, spec_norm)
        # self.up_1 = DenseBlock(self.out_dim * 4, self.out_dim * 4, act_fn_2, spec_norm)
        # self.deconv_2 = conv_trans_block(self.out_dim * 4, self.out_dim * 2, act_fn_2, spec_norm)
        # self.up_2 = DenseBlock(self.out_dim * 2, self.out_dim * 2, act_fn_2, spec_norm)
        # self.deconv_3 = conv_trans_block(self.out_dim * 2, self.out_dim, act_fn_2, spec_norm)
        # self.up_3 = DenseBlock(self.out_dim, self.out_dim, act_fn_2, spec_norm)

        self.deconv_1 = conv_trans_block(self.out_dim * 8, self.out_dim * 8, act_fn_2, spec_norm)
    ...
    def forward(self, input):
        down_1 = self.down_1(input)  # 256x256x64
        pool_1 = self.pool_1(down_1)  # 128x128x64
        down_2 = self.down_2(pool_1)  # 128x128x128
        pool_2 = self.pool_2(down_2)  # 64x64x128
        down_3 = self.down_3(pool_2)  # 64x64x256
        pool_3 = self.pool_3(down_3)  # 32x32x256
        down_4 = self.down_4(pool_3)  # 32x32x512
        pool_4 = self.pool_4(down_4)  # 16x16x512

        bridge = self.bridge(pool_4)  # 4x4x1024
        ...
        # pool_3 = self.pool_3(down_3)  # 50x50x256
        #
        # # attn_1 = self.attn_1(down_1)
        # # attn_2 = self.attn_2(down_2)
        # # attn_3 = self.attn_3(down_3)
        #
        # bridge = self.bridge(pool_3)  # 25x25x512
        #
        # deconv_1 = self.deconv_1(bridge)  # 50x50x512
        # skip_1 = (deconv_1 + down_3) / 2
        # up_1 = self.up_1(skip_1)
        # deconv_2 = self.deconv_2(up_1)  # 100x100x256
        # skip_2 = (deconv_2 + down_2) / 2
        # up_2 = self.up_2(skip_2)
        # deconv_3 = self.deconv_3(up_2)  # 200x200x128
        # skip_3 = (deconv_3 + down_1) / 2
        # up_3 = self.up_3(skip_3)
        #
        # out = self.out(up_3)  # 400x400x3 output
        # out = self.out_2(out)  # tanh
...
class DenseBlock(nn.Module):
    def __init__(self, in_dim, out_dim, act_fn, spec_norm=True):
        ...
        self.conv_1 = spectral_norm(nn.Conv2d(self.in_dim,self.out_dim, kernel_size=3, stride=1, padding=1))
        self.conv_2 = nn.Sequential(self.act_fn, spectral_norm(nn.Conv2d(self.out_dim,self.out_dim, kernel_size=3, stride=1, padding=1)),)
        self.conv_3 = nn.Sequential(self.act_fn, spectral_norm(nn.Conv2d(self.out_dim, self.out_dim, kernel_size=3, stride=1, padding=1)),)
        self.conv_4 = nn.Sequential(self.act_fn, spectral_norm(nn.Conv2d(self.out_dim, self.out_dim, kernel_size=3, stride=1, padding=1)),)
        self.conv_5 = nn.Sequential(self.act_fn,
                                    spectral_norm(nn.Conv2d(self.out_dim,self.out_dim, kernel_size=3, stride=1, padding=1)),
                                    self.act_fn,
                                    )
    ...
    def forward(self, input):
        conv_1 = self.conv_1(input)
        conv_2 = self.conv_2(conv_1)
        conv_3 = self.conv_3(conv_1 + conv_2)
        conv_4 = self.conv_4(conv_1 + conv_2 + conv_3)
        conv_5 = self.conv_5(conv_1 + conv_2 + conv_3 + conv_4)
        return conv_5
